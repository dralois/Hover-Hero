// Variant of dissolve shader, for transparent objects
Shader "Custom/DissolveShaderTransp"
{
	Properties
	{
		[Header(Dissolve)]
		_DissolveFrom ("Dissolve Begin", Range(0, 100)) = 10.0
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag(v2f i) : SV_Target
			{
				...
				float alpha = (dist < _DissolveFrom ? saturate(lerp(0, 1, dist / _DissolveFrom)) : 1) * _Color.a * finalCol.a;
		...
		Pass
		{
			...
			fixed4 frag(v2f i) : SV_Target
			{
				...
				finalCol.rgb *= alpha;
				// If close enough slowly fade to transparent
				return fixed4(finalCol.rgb + emCol.rgb, 1);
				#else
