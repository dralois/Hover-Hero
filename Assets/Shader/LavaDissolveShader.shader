Shader "Custom/LavaDissolveShader"
{
	Properties
	{
		[Header(Basics)]
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag(v2f v) : SV_Target
			{
				...
				float3 viewDir = normalize(UnityWorldSpaceViewDir(v.worldPos.xyz));
				...
				float dist = distance(v.worldPos.xz, _WorldSpaceCameraPos.xz + (viewDir.xz * _DissolveOffset));
				float alpha = (dist < _DissolveFrom ? saturate(lerp(0, 1, dist / _DissolveFrom)) : 1);
				// Return interpolated color with alpha
				return fixed4(lerp(col1, col2, abs(0.5 - phase0) / 0.5).rgb, alpha);
			}
