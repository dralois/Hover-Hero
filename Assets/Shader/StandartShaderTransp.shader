// Standart opaque shader, supports one light, blinn-phong
Shader "Custom/StandartShaderTransp"
{
	Properties
	{
		[Header(Basics)]
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag(v2f i) : SV_Target
			{
				...
				return fixed4(finalCol.rgb + emCol.rgb, _Color.a * finalCol.a);
			}
		...
		Pass
		{
			...
			fixed4 frag(v2f i) : SV_Target
			{
				...
				// Add emission and return alpha multiplied color
				fixed4 emCol = tex2D(_EmissionMap, i.uv) * _EmissionCol * _EmissionInt;
				return fixed4(finalCol + emCol * _Color.a * finalCol.a);
				#else
